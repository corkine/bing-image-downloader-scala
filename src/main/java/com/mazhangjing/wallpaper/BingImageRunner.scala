...
object BingImageRunner {
  ...
  def Try[T](op: => Option[T]): Option[T] =
    try { op } catch { case e: Throwable => logger.warn(e.getMessage); None }
  ...
  def doSaveFile(index:Int, fileName:String): Unit = {
    Try { BingImageUtils.getBingTodayImageUrl(index) } match {
      case None => logger.warn("Can't Resolve URL")
      case Some(url) =>
        logger.info(s"Get File From URL: $url now...")
        Try { BingImageUtils.getImageFromFile(url, Paths.get(fileName).toFile) } match {
